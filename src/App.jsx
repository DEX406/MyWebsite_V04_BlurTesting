...
export default function App() {
  ...
  const syncOverlays = useCallback((overlays, panX, panY, zoom) => {
    ...
    // Only regular media overlays — blur-video overlays are no longer needed
    // because media frames are now rendered directly into blur textures on the GPU.
  ...
  useEffect(() => {
    drawBgRef.current = () => {
      ...
      // Provide live DOM media elements so the GPU can copy video/GIF frames
      // directly into blur textures (eliminates soft matte edge artifacts).
      if (webgl.rendererRef.current) {
        webgl.rendererRef.current.setMediaElements(overlayElsRef.current);
      }
